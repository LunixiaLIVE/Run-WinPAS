...
### CODE START
    ...
    }elseif((Test-Path -Path $SpeedTestExe) -and !$VersionFileExists){
    ...
    if($exeMissing -or ($CurrentVersion -ne $LatestVersion)){
        Write-Host "--------------------------------"; 
        Write-Host "Missing Speedtest.exe in $WorkingDirectory";
        Write-Host "Or existing version could not be verified.";
        ...
        switch($result.ToUpper()) {
            ...
            "N"{
                Write-Warning "Quitting, you no download needed files :("; 
                if(!(Test-Path $SpeedTestExe)){ return; };
            }
            default{
